<template>
  <div v-if="video && validVideo && video.$.url && fileEsiste && video.ascolti_rimanenti" class="self-center">
    ...
    <video :src="mySource" ref="myVideo" :width="props.width" :height="props.height">
    </video>
    <q-toolbar class="mt-3">
      <q-toolbar-title class="text-sm font-semibold ">
        Visioni rimanenti: {{ ascolti_rimanenti }} - {{ duration }}/{{ elapsed }}
  ...
  <div v-if="video && video.$.url && video.$.url != 'nessuno' && !fileEsiste" class="self-center">
    <q-toolbar class="q-pa-sm text-white bg-red">
      <q-toolbar-title>
        Video {{ mySource }} non disponibile
      </q-toolbar-title>
    </q-toolbar>
  </div>
...
const mySource = computed(() => {
  ...
  // return 'fake'
  if (props.video?.$.url.endsWith('.mp4') || props.video?.$.url.endsWith('.MP4'))
